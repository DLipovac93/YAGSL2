...
public final class Autos {
  ...
  public static CommandBase driveAndSpin(SwerveSubsystem swerve) {
    return Commands.sequence(
        new RepeatCommand(
            new InstantCommand(() -> swerve.drive(new Translation2d(1, 0), 5, true), swerve)));
  }
  ...
  public static CommandBase exampleAuto(SwerveSubsystem swerve) {
    ...
    if (onTheFly) {
      // Simple path with holonomic rotation. Stationary start/end. Max velocity of 4 m/s and max
      // accel of 3 m/s^2
      example =
          PathPlanner.generatePath(
              ...
              new PathPoint(
                  new Translation2d(3, 5), Rotation2d.fromDegrees(90), Rotation2d.fromDegrees(90)),
              // position, heading(direction of travel), holonomic rotation
              new PathPoint(
                  new Translation2d(5, 5), Rotation2d.fromDegrees(0), Rotation2d.fromDegrees(0))
              // position, heading(direction of travel), holonomic rotation
              );
    } else {
  ...
  public static CommandBase driveToAprilTag(
    ...
    PathPlannerTrajectory path =
        PathPlanner.generatePath(
            ...
            new PathPoint(
                aprilTagField.getTagPose(id).get().getTranslation().toTranslation2d().plus(offset),
                rotation,
                holonomicRotation));
